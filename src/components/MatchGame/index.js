...
class MatchGame extends Component {
  ...
  getShuffledImageList = () => {
    const shuffledImageList = imagesList.sort(() => Math.random() - 0.5)

    return shuffledImageList[0]
  }

  onClickThumbItem = imageId => {
    const {activeImageList} = this.state
    if (activeImageList.id === imageId) {
      this.setState({activeImageList: this.getShuffledImageList()})
  ...
  renderGameMode = () => {
    ...
    const matchImage = activeImageList.imageUrl

    return (
      <div className="content-cont">
        <img className="shuffled-image" src={matchImage} alt="match" />
        <ul className="tab-list-cont">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isTabClicked={eachTab.tabId === activeTabId}
            />
          ))}
        </ul>
        <ul className="list-cont">
          {filteredImageList.map(eachImage => (
            <ImageListItem
              ...
              key={eachImage.id}
              onClickThumbItem={this.onClickThumbItem}
            />
          ))}
        </ul>
      </div>
  ...
  renderGameOverMode = () => {
    ...
    return (
      <div className="game-over-content-cont">
        <div className="trophy-score-button-cont">
          <img
            className="trophy"
            src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
            alt="trophy"
          />
          <p className="h-your-score">YOUR SCORE</p>
          <p className="score-in-game-over-mode">{score}</p>
          <button
  ...
  render() {
    const {timeElapsedInSeconds, isTimerRunning, score} = this.state
    return (
            ...
            <li className="nav-list-item">
              <div className="score-cont">
                ...
                <img
                  ...
                  alt="timer"
                />
                <p className="nav-bar-sec-text">{timeElapsedInSeconds} Sec</p>
              </div>
            </li>
          </ul>
        </nav>
        {isTimerRunning ? this.renderGameMode() : this.renderGameOverMode()}
      </div>
    )
  }
}
